...
import '../Assisstants/requestAssistants.dart';
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  List<PlacePredcitions> placePredictionsList = [];
  ...
  void findPlace(String placeName) async {
    if (placeName.length > 1) {
      String autoCompleteUrl =
          "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$placeName+Amphitheatre&key=$mapKey&sessiontoken=1234567890&components=country:pk";
      ...
      if (res["status"] == "OK") {
        var predictions = res["predictioins"];

        var placesList = (predictions as List)
            .map((e) => PlacePredcitions.fromJson(e))
            .toList();
        setState(() {
...
class PredictionTile extends StatelessWidget {
  final PlacePredcitions placePredictions;
  ...
  Widget build(BuildContext context) {
    return FlatButton(
      ...
      onPressed: () {
        getPlaceAddressDetials(placePredictions.placeId, context);
      },
      child: Container(
        child: Column(
          children: [
  ...
  void getPlaceAddressDetials(String placeId, context) async {
    showDialog(
      context: context,
